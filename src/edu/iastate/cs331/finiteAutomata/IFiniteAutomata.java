...
/**
 * Interface to outline basic behavior for finite automata. Alphabets for finite
 * automata are always sets of Strings to reduce complexity and align with what
 * happens in practice.
 * 
 * @author Brandon
 * 
...
public interface IFiniteAutomata {
	 ...
	 * 
	 * @return The alphabet associated with this FiniteAutomata
	 */
	...
	/**
	 * Method to check whether a given state exists in the Finite Automata
	 * 
	 * @param sid
	 * @return
	 */
	...
	/**
	 * Adds a directed edge from the state identified as fromState, to the state
	 * identified as toState, with transition as the character in the alphabet
	 * linking the two states.
	 * 
	 * For example: FA.addTransition(A, AB, B)
	 * 
	 * Results in a FA that looks like:
	 * 
	 * B (A)---->(AB)
	 * 
	 * Meaning if we are at state (A) and a B is read, we will transition to
	 * state (AB)
	 * 
	 * @param fsid
	...
	/**
	 * Checks whether the transition from fsid to tsid through the AlphabetChar
	 * transition, is contained in this Finite Automata
	 * @param fsid
	 * @param tsid
	 * @param transition
	 */
	...
	/**
	 * Removes the state identified by name from this finite automata. Any other
	 * states linking to the state to be removed will be updated to have those
	 * transitions deleted as well.
	 * 
	 * @param sid
	...
	/**
	 * Removes the transition linking fromState to another state through the
	 ...
	 *             if the the given state doesn't define a transition for the
	 *             given AlphabetChar
	 */
	public void removeTransition(StateIdentification fsid,
			AlphabetChar transition);

	/**
	 * Removes all states and transitions from this finite automata. Leaves the
	 * alphabet in tact.
	 */
	public void clearFiniteAutomata();

	/**
	 * Sets this finite automata's alphabet. Returns false if a finite automata
	 * is not empty.
	 * 
	 * @param alphabet
	...
	/**
	 * Removes the state with the given state identification from being
	 * accepting
	 * 
	 * @param sid
	 */
	...
	/**
	 * Sets the state with the given state identification as starting
	 * 
	 * @param startingSid
	 * @throws StateNotDefinedException
	 *             if the state with the given identification can't be found
	...
	/**
	 * Removes the state with the given state identification from being a start
	 * state.
	 * 
	 * @param sid
	 */
